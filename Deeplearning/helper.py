...
class writer_gui( ):
    def __init__(self, graph, share=False):
        ...
        self.thread = {
            "configurable": {
                ...
                "thread_ts": str(time.time())   # ✅ REQUIRED
            }
        }
        #self.sdisps = {} #global    
    ...
    def update_hist_pd(self,):
        ...
        for state in self.graph.get_state_history(self.thread):
            ...
            thread_ts = configurable.get('thread_ts', 'N/A')
            tid = configurable.get('thread_id', 'N/A')
    ...
    def find_config(self,thread_ts):
        for state in self.graph.get_state_history(self.thread):
            config = state.config
            if config.get('configurable', {}).get('thread_ts') == thread_ts:
                return config
        return(None)
    ...
    def copy_state(self,hist_str):
        ...
        configurable = new_state.config.get('configurable', {})
        new_thread_ts = configurable.get('thread_ts', 'N/A')
        tid = configurable.get('thread_id', 'N/A')
            ...
            def updt_disp():
                ...
                for state in self.graph.get_state_history(self.thread):
                    ...
                    if state.metadata.get('step', 0) < 1:
                        continue
                    configurable = state.config.get('configurable', {})
                    s_thread_ts = configurable.get('thread_ts', 'N/A')
                    s_tid = configurable.get('thread_id', 'N/A')
